...
model, preprocess = clip.load("ViT-B/32", device=0)
checkpoint_path = "/data/shihaitao/code/3_2021-NeurIPS-NCR-main/output_finetune_CLIP_0.5/COCO_model_para7_6.pkl"
...
# 读数据的路径
data_path = "/data/shihaitao/dataset/NCR-data/data/coco_precomp"
vocab_path = "/data/shihaitao/dataset/NCR-data/vocab"
data_split = "testall"

picturePath = "/data/shihaitao/dataset/COCO_images/COCO_test_ims"
sortedImageName = "/data/shihaitao/dataset/COCO_test_names.txt"
...
image_features_all = torch.tensor(zero_image, dtype=torch.float32, device=3)
text_features_all = torch.tensor(zero_text, dtype=torch.float32, device=3)
...
# 每次计算 1000 个测试数据的结果，最后求平均（5折交叉验证）
results = []
for i in range(5):
    image_feature = image_features_all[i * 1000: (i + 1) * 1000]
    text_feature = text_features_all[i * 5000: (i + 1) * 5000]
    sim = image_feature.cpu().numpy()@text_feature.cpu().numpy().T
    r, rt0 = i2t(
        1000, sim, per_captions=5, return_ranks=True)
