...
# 创建模型
pretrained_model_name = "/root/sdusht/huggingface/clip-vit-large-patch14-336"
model = mm_model(pretrained_model_name)
model.to(device)
processor = AutoProcessor.from_pretrained(pretrained_model_name)
# checkpoint_path = "/root/sdusht/data/3_2021-NeurIPS-NCR-main/new_output_model_CLIP/finetune_withGMM_adapter/flickr/0.4/model_best.pth.tar"
...
def load_data(data_split, data_name):
    if data_split == "test":
        if data_name =="flickr":
            with open('/root/sdusht/data/3_2021-NeurIPS-NCR-main/CLIPFinetune/data/CUHK_PEDES_image_caption_test.json', 'r', encoding='utf8') as fp:
                df = json.load(fp)
...
print("begining test!")
j_sim = np.zeros((100, 600))
i_sim = np.zeros((100, 6000))
...
r, rt = i2t(3000, similarity, 2, return_ranks=True)
ri, rti = t2i(3000, similarity, 2, return_ranks=True)
