...
# checkpoint_path = "/root/sdusht/data/3_2021-NeurIPS-NCR-main/new_output_model_CLIP/finetune_withGMM_adapter/CC/new/model_best.pth.tar"
...
def load_data(data_split, data_name):
    if data_split == "test":
        if data_name =="CC":
            with open('/root/sdusht/data/3_2021-NeurIPS-NCR-main/CLIPFinetune/data/cc152k_test.json', 'r', encoding='utf8') as fp:
                df = json.load(fp)

    return df

test_data = load_data("test", "CC")
...
for i in range(10):
    ...
    for j in range(2):
        text_start = 500 * j
        text_end = 500 * (j + 1)
        model_input = processor(text=text_inputs[text_start: text_end], images=image_inputs[image_start: image_end],
