...
from NCR.model import SGRAF
from collections import OrderedDict
from NCR.utils import AverageMeter, ProgressMeter
from NCR.data import get_dataset, get_loader
...
def i2t(npts, sims, per_captions=1, return_ranks=False):
    ...
    for index in range(npts):
        inds = np.argsort(sims[index])[::-1]  #返回每一个image和所有text相似性的取值的 下标从大到小排序
        retreivaled_index.append(inds)
        # Score
        rank = 1e20
        for i in range(per_captions * index, per_captions * index + per_captions, 1):
            tmp = np.where(inds == i)[0][0]   # np.where返回的类型是tuple，tuple里面是list，因此[0][0]是取到最内层list里面的值
            if tmp < rank:
                rank = tmp
        ranks[index] = rank
        top1[index] = inds[0]
...
if __name__ == "__main__":
    ...
    model_path = "/root/sdusht/data/3_2021-NeurIPS-NCR-main/output/Pre-trained_best model/ncr_f30k_0_model_best.pth.tar"
    data_path = "/root/sdusht/dataset/NCR-data/data"
